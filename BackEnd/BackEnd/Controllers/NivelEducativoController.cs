...
namespace BackEnd.Controllers
{
    ...
    public class NivelEducativoController : ControllerBase
    {
            ...
            public NivelEducativoController(SomosdcContext context)
            {
                _context = context;
            }

            [HttpGet]
            public async Task<ActionResult> getNivelEducativos()
            {
                try
                {
                    var lista = _context.TblNivelEducacions.Where(x => x.EstadoEliminacion == 0).ToList();
                    if (lista.Count > 0)
                    {
                        return Ok(new
                        {
